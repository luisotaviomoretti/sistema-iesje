-- =====================================================
-- MIGRATION: 026_fix_enroll_finalize_return_type.sql
-- DESCRIÇÃO: Ajusta o tipo de retorno de discount_count para bigint,
--            alinhando com COUNT(*) (bigint) e evitando erro 42804.
-- DATA: 2025-09-09
-- =====================================================

-- É necessário dropar e recriar a função para alterar o tipo de retorno
DROP FUNCTION IF EXISTS public.enroll_finalize(jsonb, jsonb, uuid);

CREATE FUNCTION public.enroll_finalize(
  p_enrollment jsonb,
...
BEGIN
  ...
  IF p_client_tx_id IS NOT NULL THEN
    ...
    IF v_existing_id IS NOT NULL THEN
      ...
      SELECT v_existing_id,
             false AS created,
             (SELECT count(*) FROM public.enrollment_discounts d WHERE d.enrollment_id = v_existing_id) AS discount_count;
      RETURN;
    END IF;
  END IF;
  ...
  SELECT v_enrollment_id,
         true AS created,
         (SELECT count(*) FROM public.enrollment_discounts ed WHERE ed.enrollment_id = v_enrollment_id) AS discount_count;
