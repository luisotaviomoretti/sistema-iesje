-- =====================================================
-- MIGRATION: 053_inadimplentes_replace_by_school.sql
-- DESCRIÇÃO: RPC transacional para substituir (por escola) a base ativa de inadimplentes
--            Operação segura: valida admin, soft desativa registros existentes e insere os novos
--            em lote a partir de JSONB. Não altera tabelas/policies existentes.
-- DATA: 2025-10-02
-- AUTOR: Cascade
-- =====================================================
...
-- RPC: replace_inadimplentes_by_school
-- Parâmetros:
--   p_escola: texto livre (ex.: 'pelicano', 'sete_setembro')
--   p_rows: jsonb array com objetos { codigo_inadim, student_name, guardian1_name, student_escola?, meses_inadim }
-- Retorno: contagem de desativados e inseridos
...
DECLARE
  ...
  v_ins int := 0;
BEGIN
  -- Requer admin (bypass RLS sob SECURITY DEFINER, mas apenas para admins)
  ...
  -- Soft desativar registros ativos da escola alvo
  ...
  to_ins AS (
    SELECT
      ...
      s.student_name,
      s.guardian1_name,
      -- Prioriza escola do payload se presente; senão usa p_escola
    ...
    FROM src s
    WHERE s.student_name IS NOT NULL -- obrigatório
  ),
  ins AS (
  ...
  -- Auditoria leve de import (nível de operação)
